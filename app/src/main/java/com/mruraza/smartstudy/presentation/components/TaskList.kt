...
fun LazyListScope.taskList(
    ...
    onTaskCardClick: (Int?) -> Unit // task id
) {
    item {
        Text(
            text = sectionTitle,
    ...
    if (task.isEmpty()) {
        item {
            Column(
                ...
                Image(
                    ...
                    contentDescription = "please Add task"
                )
                Text(
                    text = "You don't have any upcoming tasks.\n"+"Click + button in subject screen to add tasks",
        ...
        items(task) { it->
            TaskCard(modifier = Modifier.fillMaxWidth().padding(horizontal = 12.dp, vertical = 4.dp),
                task = it,
